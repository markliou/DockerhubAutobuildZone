...
void Solver<Dtype>::Step(int iters) {
  ...
  std::string log_directory = param_.snapshot_prefix().substr(0, param_.snapshot_prefix().find_last_of("/"));    // Oscar
  ...
  test_log_.open(test_log_path.string().c_str(), std::ofstream::out);                                           // Oscar
  ...
  while (iter_ < stop_iter) {
    ...
    if (display) {
      LOG_IF(INFO, Caffe::root_solver()) << "Iteration " << iter_
          << ", loss = " << smoothed_loss_;
      train_log_ << iter_ << ", " << smoothed_loss_ << std::endl;             // Oscar
  ...
  train_log_.close();                     // Oscar
  test_log_.close();                      // Oscar
}
...
void Solver<Dtype>::TestDetection(const int test_net_id) {
  ...
  if (param_.test_compute_loss()) {
    loss /= param_.test_iter(test_net_id);
    test_log_ << iter_ << ", " << loss << std::endl;                        // Oscar
  }
  for (int i = 0; i < all_true_pos.size(); ++i) {
    ...
    const string& output_name = test_net->blob_names()[output_blob_index];
    LOG(INFO) << "    Test net output #" << i << ": " << "loss = " << loss << ", " << output_name << " = "           // Oscar
              << mAP << "\n\n";
  }
}
